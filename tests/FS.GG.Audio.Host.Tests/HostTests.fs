    ...
    testList
        ...
        [
            ...
            test "requiresEngine is true for exactly the effects a raw backend cannot realize (#27)" {
                ...
                Expect.isTrue
                    (HostAudio.requiresEngine (CoreAudio.setBusVolume Music 0.25))
                    "SetBusVolume is dropped raw"

                Expect.isTrue (HostAudio.requiresEngine (CoreAudio.duck Music 0.5 200.0)) "Duck is dropped raw"

                Expect.isTrue
                    (HostAudio.requiresEngine (CoreAudio.playSfx3D (SoundId "s") 1.0 0.0 0.0 1.0))
                    "PlaySfx3D degrades raw"
                ...
                Expect.isFalse
                    (HostAudio.requiresEngine (CoreAudio.setMasterVolume 0.5))
                    "SetMasterVolume is honoured raw"
            }
            ...
            testSequenced (
                test "Audio.play names the silent drop once, and still plays the batch (#27)" {
                    ...
                    let effects =
                        [ CoreAudio.setBusVolume Music 0.25; CoreAudio.playSfx (SoundId "s") 1.0 ]
                    ...
                    let occurrences =
                        text.Split([| "reached IAudioBackend directly" |], System.StringSplitOptions.None).Length
                        - 1
                    ...
                    Expect.equal
                        fake.Calls
                        (effects @ effects)
                        "every effect still reaches the backend in dispatch order"
                }
            )
            ...
            test "an IAudioBackend this library did not build is Unknown, never DeviceBacked (#34)" {
                ...
                Expect.equal
                    (Backend.kindOf fake)
                    BackendKind.Unknown
                    "a foreign backend is Unknown, not assumed audible"
            ...
            test "NullBackend records in constant cost per play, whatever it already holds (report §3.3)" {
                ...
                for _ in 1..9 do
                    batchAlloc n |> ignore // bury it under 18k more
            ...
            test "NullBackend diagnostics are raw, bounded, and independently cleared (#199)" {
                ...
                Expect.equal nb.Evidence (CoreAudio.interpret [ raw ]) "evidence remains the normalized audit trail"
            ...
            test "a substituted Null fallback keeps only bounded recent diagnostics, not audit evidence (#114, #199)" {
                ...
                let expected =
                    [
                        for i in total - NullBackend.DiagnosticCapacity + 1 .. total -> PlaySfx(SoundId(string i), 5.0)
                    ]
            ...
            test "reading NullBackend.Evidence never throws while another thread is playing (report §3.3)" {
                ...
                use stop =
                    new System.Threading.CancellationTokenSource(System.TimeSpan.FromMilliseconds 500.0)
            ...
            test "OpenAlBackend.create degrades to a Null it MARKS as substituted, and never throws (FR-004, #34)" {
                ...
                try
                    ...
                    | BackendKind.DeviceBacked ->
                        ...
                        Expect.isTrue
                            (backend :? IMixingBackend)
                            "a real device backend implements IMixingBackend (#11)"
            ...
            test "BufferCache keys handles by id and lists every one for disposal (#20)" {
                ...
                Expect.equal
                    (Set.ofArray cache.Handles)
                    (Set.ofList [ 1u; 2u ])
                    "every uploaded handle is listed for deletion"
            }
            ...
            test "BufferCache does not cache a failed upload, so a later resolve still populates (#20)" {
                ...
                Expect.equal
                    (cache.GetOrAdd(SoundId "s", create))
                    (Some 7u)
                    "a later successful resolve populates the entry"
            ...
            test "VoicePool DisposeAll stops and deletes every handle it owns (#20)" {
                ...
                Expect.equal
                    (Set.ofList fake.StopCalls)
                    (Set.ofList [ a; b ])
                    "each sounding voice is stopped before deletion"
            ...
            test "a missing asset is named, not swallowed — the id and the resolver that failed (#28)" {
                ...
                let line =
                    AssetDiagnostics.message (AssetDiagnostics.Sound(SoundId "explosion")) AssetDiagnostics.Unresolved
                ...
                Expect.stringContains
                    line
                    "silent"
                    "...and the consequence, so it is not mistaken for a warning about nothing"
                ...
                Expect.stringContains
                    line
                    "AssetResolver.ResolveSound"
                    "...and points at the resolver the product supplies"
            }

            test "a missing track names the track resolver, not the sound one (#28)" {
                let line =
                    AssetDiagnostics.message (AssetDiagnostics.Track(TrackId "theme")) AssetDiagnostics.Unresolved
            ...
            test "an undecodable asset is distinguished from a missing one (#28)" {
                ...
                let notWav =
                    AssetDiagnostics.message (AssetDiagnostics.Sound(SoundId "s")) (AssetDiagnostics.NotWav 1234)
                ...
                Expect.stringContains
                    notWav
                    "1234"
                    "...and how many bytes came back, so an HTML 404 body is recognisable"

                let badFormat =
                    AssetDiagnostics.message
                        (AssetDiagnostics.Sound(SoundId "s"))
                        (AssetDiagnostics.UnsupportedFormat(6, 24))
                ...
                test "a 3D voice reaches the device with the distance model OFF and the gain we asked for (report §5)" {
                    ...
                    try
                        ...
                        | BackendKind.DeviceBacked ->
                            ...
                            Expect.isTrue
                                relative
                                "the source is listener-relative, so its position reads as a direction"

                            Expect.equal
                                rolloff
                                0.0f
                                "the device's own distance model is OFF — the Engine already attenuated"

                            Expect.floatClose
                                Accuracy.high
                                (float gain)
                                0.5
                                "the gain we passed is the gain the device holds, un-attenuated"
                    ...
                    try
                        ...
                        | BackendKind.DeviceBacked ->
                            ...
                            Expect.stringContains
                                ...
                                "under the operation that actually failed"
            ...
            test "DeviceDiagnostics.message names the operation, the degrade, and the way back (#33)" {
                ...
                let transient =
                    DeviceDiagnostics.message DeviceDiagnostics.PlayAt DeviceDiagnostics.Transient error

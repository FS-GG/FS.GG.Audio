...
let api =
    createObj
        [
            ...
            "loadSound"
            ==> fun (id: string) (url: string) -> host.Value.LoadSound(SoundId id, url)
            "loadTrack"
            ==> fun (id: string) (url: string) -> host.Value.LoadTrack(TrackId id, url)
            "play"
            ==> fun (id: string) ->
                host.Value.Dispatch(PlaySfx(SoundId id, 0.8))
                observe ()
            "playAt"
            ==> fun (id: string) ->
                host.Value.Dispatch(PlaySfx3D(SoundId id, 1.0, 0.0, 1.0, 0.75))
                observe ()
            "music"
            ==> fun (id: string) ->
                host.Value.Dispatch(PlayMusic(TrackId id, true))
                observe ()
            "stopMusic"
            ==> fun () ->
                host.Value.Dispatch StopMusic
                observe ()
            "master"
            ==> fun (gain: float) ->
                host.Value.Dispatch(SetMasterVolume gain)
                observe ()
            "bus"
            ==> fun (gain: float) ->
                host.Value.Dispatch(SetBusVolume(Ui, gain))
                observe ()
            "duck"
            ==> fun () ->
                host.Value.Dispatch(Duck(Music, 0.5, 100.0))
                observe ()
            "pause"
            ==> fun () ->
                host.Value.Pause()
            ...
            "dispose"
            ==> fun () ->
                (host.Value :> IDisposable).Dispose()
                observe ()
        ]

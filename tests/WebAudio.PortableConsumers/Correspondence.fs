...
let private status =
    function
    | WebAudioStatus.Locked -> "locked"
    | WebAudioStatus.Running -> "running"
    | WebAudioStatus.Paused -> "paused"
    | WebAudioStatus.Disposed -> "disposed"
...
let run () =
    ...
    let _, rows =
        observations
        |> List.fold
            (fun (state, rows) observation ->
                let next, effects = WebAudioPolicy.update config observation state

                let currentStatus, assets, ready, voices, music, nextId =
                    WebAudioPolicy.observe next

                let musicState = if music then "music" else "silent"
                let effectText = effects |> List.map effect |> String.concat ","

                let row =
                    $"{status currentStatus}|{assets}|{ready}|{voices}|{musicState}|{nextId}|{effectText}"

...
let tests =
    testList
        ...
        [
            ...
            test
                "the project declares exactly Elmish + Core/Host/Engine + FSharp.Core, never FS.GG.UI/SkiaSharp (FR-004)" {
            ...
            test "ofEngine routes the batch through Engine.step so mixing applies (005 FR-001, FR-002)" {
                ...
                let cmd: Elmish.Cmd<int> =
                    AudioCmd.ofEngine engine 0.0 [ CoreAudio.setBusVolume Sfx 0.5; CoreAudio.playSfx (SoundId "s") 1.0 ]
            ...
            testSequenced (
                test
                    "ofEffects vs ofEngine on the same batch: raw drops — and now SAYS so, once — vs mixed (#29, 005 FR-002)" {
                    ...
                    let diagnostics (s: string) =
                        s.Split([| "reached IAudioBackend directly" |], System.StringSplitOptions.None).Length
                        - 1
            ...
            test "ofEngine carries 3D positioning through the Cmd path on a mixing backend (005 FR-002)" {
                ...
                run (AudioCmd.ofEngine engine 0.0 [ CoreAudio.playSfx3D (SoundId "s") 3.0 0.0 0.0 1.0 ])
                |> ignore
            ...
            test "the committed .fsi surface baseline matches the source signature, no drift (FR-006)" {
                ...
                let baseline =
                    Path.Combine(root, "docs", "api-surface", "FS.GG.Audio.Elmish", "Elmish.fsi")

...
let tests =
    testList
        "Feature 243 Audio request surface + record-only interpreter (US1, US2)"
        [
            ...
            test "smart constructors clamp carried volume into [0,1] (US1)" {
                Expect.equal
                    (Audio.playSfx (SoundId "x") 2.0)
                    (PlaySfx(SoundId "x", 1.0))
                    "over-max sfx volume clamps to 1.0"

                Expect.equal
                    (Audio.playSfx (SoundId "x") -0.5)
                    (PlaySfx(SoundId "x", 0.0))
                    "below-min sfx volume clamps to 0.0"
            ...
            testCase "clampVolume result is always within [minVolume, maxVolume] (FsCheck >=1000 cases)"
            <| fun () ->
            ...
            testCase "interpret preserves count and normalizes recorded volumes (FsCheck >=500 cases)"
            <| fun () ->

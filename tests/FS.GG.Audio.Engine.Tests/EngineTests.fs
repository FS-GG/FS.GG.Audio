...
let private repoRoot () =
    ...
    if isNull dir then
        failwith "repo root (FS.GG.Audio.slnx) not found"
    else
        dir.FullName
    ...
    testList
        ...
        [
            ...
            test "crossFade scales by and restores configured source/target gains (#197)" {
                ...
                Engine.step engine 0.0 [ CoreAudio.setBusVolume Music 0.4; CoreAudio.setBusVolume Sfx 0.25 ]
            ...
            test "crossFade preserves a muted target through and after completion (#197)" {
                let engine = Engine.create (mixing () :> IAudioBackend)
                Engine.step engine 0.0 [ CoreAudio.setBusVolume Music 0.6; CoreAudio.setBusVolume Ambient 0.0 ]
                ...
                Expect.floatClose
                    acc
                    (engine.BusGain Ambient)
                    0.0
                    "completion does not overwrite target mute with unity"
            ...
            test "a nan fade duration sets the gain at once, it does not silence the bus forever (report §3.6)" {
                ...
                Expect.floatClose
                    ...
                    (engine.BusGain Music)
                    1.0
                    "and it stays there — no stuck fade rendering a nan gain"
                ...
                for _ in 1..100 do
                    Engine.step e2 0.016 []

                Expect.floatClose
                    acc
                    (e2.BusGain Music)
                    0.0
                    "crossFade with a nan duration completes at once: from -> 0"
            ...
            test "an infinite fade duration holds the bus, rather than snapping it to the target (report §3.6)" {
                ...
                Expect.floatClose
                    ...
                    (engine.BusGain Music)
                    0.8
                    "an infinitely slow fade-out never arrives, and never jumps"
            }
            ...
            test "3D: pan is monotonic in azimuth and total on nonsense positions (FR-005)" {
                ...
                let bearings =
                    [ 0.0..5.0..90.0 ] |> List.map (fun deg -> deg * System.Math.PI / 180.0)
                ...
                Expect.isGreaterThan
                    (List.last pans - List.head pans)
                    0.9
                    "and it spans the range rather than saturating early"
                ...
                let ev =
                    CoreAudio.interpret
                        [
                            ...
                            CoreAudio.duck Music 2.0 -5.0
                        ] // amount 2.0 -> 1.0, ms -5 -> 0
            ...
            test "a transient hiccup degrades to silence and is never called a dead device (#33, FR-008)" {
                ...
                let engine =
                    Engine.createWithDiagnostics device Engine.defaultSpatial (backend :> IAudioBackend)
            ...
            test "a healthy backend says nothing at all (#33)" {
                ...
                let engine =
                    Engine.createWithDiagnostics device Engine.defaultSpatial (mixing () :> IAudioBackend)

                for _ in 1..100 do
                    Engine.step engine 0.016 [ CoreAudio.playSfx (SoundId "s") 1.0 ]
            ...
            test "a dead device is escalated even when most frames carry no audio (#33)" {
                ...
                for i in 1..600 do
                    let frame =
                        if i % 3 = 0 then
                            [ CoreAudio.playSfx (SoundId "step") 1.0 ]
                        else
                            []
            ...
            test "a sink keeps a 3D voice positional, where the raw path degrades it (#27)" {
                ...
                Expect.floatClose
                    ...
                    v.EffectiveGain
                    (1.0 / 3.0)
                    "inverse-distance attenuation at 3x the reference distance"
            ...
            test "committed .fsi baselines match the sources, no drift (FR-009)" {
                ...
                let check pkg name =
                    ...
                    Expect.equal
                        ...
                        (sprintf "%s surface matches baseline" pkg)
